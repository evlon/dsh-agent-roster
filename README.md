...
roster 服务端**自身不感知域名**——它只监听 `ROSTER_HOST:ROSTER_PORT`，域名由反向代理
（Caddy / Higress）决定，因此服务端无需任何域名配置。使用方按域名填 `ROSTER_URL` 即可：

| 用途 | 旧环境 | 新环境（K8S） |
|---|---|---|
| 花名册服务 | `http://roster.example.com` | `http://roster.example.com` |

其他服务对应关系：门户 `market.example.com` → `market.example.com`；
岗位网关 `job.example.com` → `job.example.com`；
配置中心 `conf.example.com` → `conf.example.com`。

`scripts/simulate.js`（压测/造数脚本）默认连新环境，可用 `ROSTER_URL` 覆盖：

```bash
node scripts/simulate.js                                   # 默认 http://roster.example.com
ROSTER_URL=http://roster.example.com node scripts/simulate.js   # 切回旧环境
```
...
# 内网已部署的实例：新 K8S 环境 http://roster.example.com（旧环境 http://roster.example.com）
