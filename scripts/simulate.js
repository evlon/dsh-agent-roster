...
// roster 服务地址：默认指向新 K8S 环境（*.example.com），新旧环境并存时可用
// ROSTER_URL 覆盖（如 ROSTER_URL=http://roster.example.com 切回旧环境）。
const BASE = process.env.ROSTER_URL || 'http://roster.example.com'

const TEAMS = ['growth', 'core', 'infra', 'data']
const ROLES = ['dev', 'dev', 'dev', 'dev', 'qa', 'qa', 'pm', 'leader', 'custom']
const OWNERS = ['@twin-owner', '@liamgnc', '@lihua', '@wangming']
